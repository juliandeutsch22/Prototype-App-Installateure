...
| **Meine Baustellen** | Die Baustellen, denen der Monteur zugeordnet ist, mit Route und Telefonnummer **Seit 24.09.2026 (Prüflauf L2):** auch die Baustellen, auf die er eingeteilt ist, mit dem nächsten Einsatztag — für den Monteur ist „eingeteilt“ dasselbe wie „zugeordnet“. | Mitarbeiter | `projects` | Ansicht (7) | Abgeschlossene fallen heraus, pausierte bleiben; ein fehlender Ansprechpartner wird angemahnt statt verschwiegen |
...
| **Datensicherung** | Nächtliche Ausleitung des ganzen Bestands an einen zweiten Ort — **samt der Fotos am Handwerksschein und der Pläne an der Baustelle**; Sicherung von Hand anstoßen; Bestand herunterladen (DSGVO); **Zustand des letzten Laufs**; Rücklauf als Werkzeug für die Hand (`scripts/ruecklauf.mjs`) | **nur GF/Admin** | alle Tabellen mit `company_id`, `system_laeufe`, `ausleitung_dateien`, Eimer `scheinfotos` und `baustellendokumente` | Rechnung (23 Aufräum-, Pfad- und Fristregeln), Zielspeicher und Signatur (24 inkl. AWS-Testvektoren), Ansicht (14), Datenbank (12 Ausleitung + 14 Dateien + 8 Wächter + 2 Rücklauf) | Ohne eingerichteten Zielspeicher liegt die Sicherung im selben Projekt — gegen einen Fehlgriff hilft das, gegen „der Zugang ist weg" nicht. **Die Fotos gehen nur ausser Haus**, weil sie im eigenen Projekt schon liegen; das Dienstkonto dort darf nur anlegen, deshalb führt `ausleitung_dateien` Buch und der Rücklauf holt die Dateien nicht selbst zurück **Nach einer Sicherung von Hand liest die Statuszeile ihren Stand neu** — vorher stand direkt über „Sicherung erstellt“ weiter „noch nie durchgelaufen“ in Warnfarbe (seit 24.09.2026). |
| **Fehlerprotokoll und „Problem melden"** | **Seit 24.09.2026.** Abstürze (Fehlergrenze) und unbehandelte Fehler (Klick, Versprechen) gehen in die eigene Tabelle `fehlerprotokoll` statt nur in die Konsole — **geputzt**: keine Formularinhalte, Text in Anführungszeichen, E-Mail-Adressen und Ziffernfolgen entfernt, Ansicht ohne Kennungen und Suchbegriff. Nachladefehler, fehlendes Netz und fremde Skripte gelten als Rauschen. Gebremst im Browser (derselbe Fehler einmal in 10 Min., höchstens 20 je Seite) und in der Datenbank (30 je Konto und Stunde). „Problem melden" in Seitenleiste, Profilblatt und auf der Fehlertafel: Beschreibung plus Ansicht, Fassung, Gerät und der Fehler von eben. **Seit 25.09.2026 geht jede Meldung an den Senklot-Support, und nur dorthin** — mit Name und E-Mail-Adresse, damit er nachfragen kann (bei einem Benutzerkonto nur der Name; die Kunstadresse erreicht niemanden). Das Häkchen „auch an den Support“ ist weg, die Datenbank setzt es selbst. Die Unterseite Einstellungen → Fehler ist entfallen und der Betrieb liest das Protokoll nicht mehr: die Geschäftsführung konnte mit Meldungen und Stapeln nichts anfangen, beheben kann es nur der Support. Wer und wann setzt die Datenbank; niemand ändert oder löscht; nach **90 Tagen** gelöscht (pg_cron). Die **Plattform** sieht über `fehlerprotokoll_plattform` die technischen Fehler aller Betriebe ohne Person und jede Meldung mit Absender | schreiben alle; lesen nur die Plattform, eng begrenzt | `fehlerprotokoll` | Datenbank (13), Rechnung (11 Putzen/Bremsen + 3 Bündeln), Ansicht (5 + 2 Fehlergrenze + 2 Plattform + 1 Hülle), Browser (Probelauf 32) | Ein Nachladefehler, bei dem auch das Neuladen nicht half, geht nicht ins Protokoll (er gilt als Rauschen). Fehler ohne Anmeldung (Anmeldeseite) werden nicht erfasst — der Zeilenschutz lässt nur Betriebsmitglieder schreiben. **Ältere Meldungen** ohne Häkchen bleiben für den Support unsichtbar, ältere mit Häkchen ohne Namen — sie wurden unter dieser Zusage geschrieben und verschwinden nach 90 Tagen. **Eine Antwort** schreibt der Support per E-Mail; in der App gibt es keinen Rückkanal |
...
| **Nummernkreise** | Vorsätze für Rechnungen, Angebote, Baustellen und den Fuhrpark — einstellbar je Betrieb, mit Vorschau der nächsten Nummer. Die Baustellennummer wird seit 18.09.2026 vorgeschlagen (Zähler wie bei Rechnung und Angebot) und bleibt überschreibbar. **Seit 24.09.2026 zieht ein stehengelassener Vorschlag mit**, wenn die Liste erst nach dem Formular eintrifft — vorher blieb „…-0001“ aus der noch leeren Liste stehen, galt beim Speichern als eigene Nummer, und die zweite Baustelle scheiterte an der Dublette. Eine vergebene Nummer wird beim Namen genannt | Rechenregeln (21), Datenbank (13) | Gelten **ab jetzt, nicht rückwirkend** — eine ausgestellte Rechnung lässt sich nach § 132 BAO nicht mehr ändern. Geprüft wird die Form auch serverseitig (`companies_praefix_*`): der Vorsatz landet im Dateinamen des PDFs und in der Buchhaltungs-CSV. `WZ-` beim Kennzeichen stand dreifach fest im Code — der Kenner eines bestimmten Bezirks auf jedem Zeiteintrag |
...
| **Fehlermeldungen beim Speichern** | Lehnt der Server ab, sagt die Maske warum: die Gründe der eigenen Datenbank unverändert, die technischen von Postgres übersetzt (doppelter Wert, fehlendes Recht, kein Netz, abgelaufene Anmeldung, fehlende Angabe), alles andere mit dem Satz der Maske — `src/lib/fehlerGrund.ts`, an rund 50 Stellen | Rechenregeln (5), Ansicht (Baustelle doppelt, Einblick in der Zeitmaske, 4 angepasste), Browser (Probelauf 24.09.2026) | **Seit 24.09.2026.** Vorher stand an fast jeder Maske „… konnte nicht gespeichert werden. Bitte erneut versuchen.“ — auch bei einer vergebenen Nummer oder einem Supportzugang, der nur lesen darf; der zweite Versuch scheiterte genauso, und niemand erfuhr warum. Im Einblick der Stufe „ansehen“ heisst ein fehlendes Recht „Im Einblick wird nur gelesen …“. **Bekannte Lücke:** die Knöpfe bleiben im Einblick sichtbar und bedienbar; gesperrt wird in der Datenbank, und die Meldung nennt den Grund. Sie alle einzeln zu sperren hiesse, rund hundert Stellen anzufassen — ein allgemeiner Riegel über alle Hauptknöpfe träfe auch PDF und Export, die der Support zum Helfen braucht |
| **Darstellung am Telefon** | Nachgemessen bei **375 px** (iPhone XS): kein Element ragt über den Rand, Listenzeilen tragen höchstens vier Elemente rechts, Unterzeilen brechen nur **zwischen** den Feldern | Eurozeichen (9), Zeilenbreite (1) | Gemessen, nicht behauptet — eine wegwerfbare Playwright-Sonde über sechs Ansichten. Zwei Befunde waren gar keine Umbrüche: ein **doppeltes Eurozeichen** (`fmtEUR` gibt es in acht Dateien, vier stellen das Zeichen voran, vier nicht — wer abschreibt, erwischt die falsche Hälfte) und das **✕ allein in der zweiten Zeile**, die einzige unumkehrbare Aktion am auffälligsten Platz. **Nicht behoben:** die dritte Kennzahl-Karte steht allein — drei Spalten schnitten „€ 22 104,60" ab, und `col-span-2` änderte nachgemessen nichts **Seit 24.09.2026 (Prüflauf, D1–D7):** ein Datumsformat überall — TT.MM.JJJJ über `lib/datum.ts`; gespeichert und in Eingabefeldern bleibt es ISO. Zahlen mit Komma („2,4 MB“). Keine Schrift mit Inhalt unter 12 px (Kartentitel, untere Leiste, Kalenderzahlen, Abzeichen) — gemessen: auch mit 12 px wird in der unteren Leiste bei 375 px bei keiner Rolle etwas abgeschnitten. Silbentrennung nach deutschen Regeln (`hyphens: auto`). Platzhalter in eigenem, hellerem Ton (4,9:1) und als Beispiel erkennbar („z. B. ATU…“). Die Einstellungen tragen Reiter wie der Rest der App — am Telefon laufen sie seitlich und der gewählte bleibt im Bild, am Schreibtisch brechen sie in eine zweite Zeile um (die Auswahlliste aus Paket 6 ist nach Rückmeldung aus dem Betrieb wieder entfallen). Kartenlinks, Rücksprünge und Personen-Chips haben Daumenhöhe. Seitenleiste ab 1024 px 280 px breit, nichts mehr gekürzt. **Bewusst geblieben:** die Unterreiter der Einstellungen, Einsatzplanung und Mein Einsatzplan stehen über der Seitenüberschrift, die Reiter in Urlaub, Material, Anforderungen und Lager darunter — die einen wechseln die Seite, die anderen die Ansicht innerhalb einer Seite. Beides an eine Stelle zu bringen hiesse, jede Unterseite umzubauen. **Seit 24.09.2026 (Prüflauf, D8–D20, F16) — lange Seiten entschlackt:** Zeitmaske des Monteurs: Wegzeit, Fahrzeug, Helfername und Zuschläge stehen hinter „Weitere Angaben“; gesetzte Werte stehen in der Zeile, offen startet sie, wenn der Eintrag oder der letzte Eintrag solche Angaben trägt; der Helfer-Haken bleibt sichtbar (Stundensatz). „Zu meinen Einträgen“ springt am Telefon zur Liste. Schein: Hinweise und Absätze gekürzt, die Foto-Erklärung nennt nicht mehr „Firebase Storage“. Einstellungen: „Sätze und Kosten“, „Nummernkreise“ und „Personal“ (Urlaubsjahr, Genehmigende, Wochenplan, Monatsbilanzen) sind eigene Unterseiten mit eigener Überschrift; ein Fehler steht an der Karte, deren Speichern scheiterte. Anforderungen: Status und Löschen im „⋯“ der Zeile, der Zustand steht einmal. Rechnungen: der Monats-Export steht am Seitenende, die Rechnungsart ist so breit wie ihr längster Eintrag. Wartungen: eine fällige Wartung trägt ihre Knöpfe nur oben unter „Steht an“. Wochenplan: am Wochenende und Feiertag kein „frei“. Urlaub: die Arbeitstage stehen erst da, wenn ein Zeitraum gewählt ist. Datensicherung mit Kartentiteln. Plattform und Willkommen mit Markenband; die Plattform rügt eine leere Maske nicht. Mitarbeiterübersicht: Marken wie „kein Eintritt hinterlegt“ stehen beim Namen. Material: kein „ohne Kategorie“ mehr, kürzere Suchplatzhalter. Das „i“ ist kein Absatz mehr im Absatz (F16). **Bewusst geblieben:** „Einteilen“ an vergangenen Tagen im Wochenplan — die Tagesplanung sperrt die Vergangenheit nicht, ein vergessener Einsatz lässt sich nachtragen. Die drei Hinweise über der Startseite (Nachtläufe, Wartungen, Tage ohne Buchung) bleiben, wo sie sind; „Tage ohne Buchung“ sieht seit Paket 4 nur, wer ein Zeitkonto führt — die Administration nie, die Geschäftsführung nur mit eingeschaltetem Zeitkonto. Der Schein ist mit allen Karten am Telefon weiter rund 3 400 px lang — Zeiten, Material, Notiz, Fotos und zwei Unterschriften sind der Beleg selbst. **Seit 24.09.2026 (Prüflauf, C1–C14) — die Farben der Marke überall:** Rechnung, Angebot, Mahnung und Einkaufsliste drucken in den Tönen der Oberfläche (`--text`, `--text-muted`, `--border`) statt in den Grautönen von Bootstrap; der Schein hat keinen Schiefergrau-Kopfbalken mehr, sondern dieselben Tabellen wie die Rechnung, und sein Leistungsdatum steht als TT.MM.JJJJ; der Stundennachweis trägt Petrol statt des alten Marineblaus. `tests/unit/markenfarben.test.ts` hält fest, dass die Belegfarben die Tokens aus `index.css` sind und die alten Werte nicht zurückkommen. Eine Farbe für Hauptaktionen (`primary`; die Variante `accent` gibt es nicht mehr). Meldungen als weisse Karte mit farbiger Kante statt vollflächig grün oder rot. Ein Abdunkler für alle Dialoge. Überschriften halbfett in normaler Laufweite, die untere Leiste halbfett. Fokusring, stärkere Linie und Lade-Platzhalter als Tokens. Die Unterschrift zieht in `--text`. **Bewusst geblieben:** fett bleibt, was einen Zustand trägt (aktiver Eintrag in Seitenleiste und Reitern) oder eine Zahl hervorhebt. **Rückmeldung aus dem Betrieb (24.09.2026):** Die aufgeklappte Mitarbeiterkarte ist ruhiger — der Saldo in Tinte und halbfett statt 32 px fett in Rot, die fehlenden Tage in Warnfarbe statt Rot direkt darunter; die Farbe des Saldos trägt der Punkt in der Kopfzeile. Das „i“ steht nie mehr in einer Flex-Zeile ohne Umbruch (Betriebsurlaub, Benutzername, Kontaktzeile beim Grosshändler, Fehlerprotokoll); `tests/unit/infoHintUmbruch.test.ts` prüft das am Syntaxbaum jeder Ansicht. |
